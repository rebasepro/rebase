...
| `check:doc-examples` | Every collection example in the English docs, evaluated and handed to the boot-time config validator. A key that moved into `admin` in 0.11 — `widget`, `defaultFilter`, `sort`, `entityActions` — still typechecks at the top level of a collection and still refuses to boot; three were live, beside an `overrides: EntityOverrides` row for a type that never existed. | `<!-- doc-examples: fragment -->` above a fence that is genuinely an excerpt |
| `check:schema-sample` | The Drizzle and SQL blocks on `architecture/schema-as-code.md` are what the generators emit for the collection shown above them. The hand-written pair had `serial`, `varchar`, `.default(true)` and `.defaultNow()` against a generator that emits none of the four. | `pnpm check:schema-sample --write` |
