...
| The guest flag, every door | **BUG, security**, reproduced and fixed (2026-09-15). A guest has a real uid, so `isAnonymous` is all that tells it from an account, and `policy.registered()` reads it. Six places dropped it, all found by guests reading rows a `registered()` rule withholds, against mongod and a real Postgres. Mongo socket: neither sign-in path stored the flag, `getScopedDelegate` wrote `isAnonymous: false`, and subscriptions sent `{ uid, roles }`. Mongo in-process listener: `AuthenticatedMongoDriver.authContext()` was `{ uid, roles }` and `scopedDriver` rebuilt the user with `as User`. `checkOperation` (common) built its policy context without it, so every Mongo row-in-hand decision (`fetchOne`, `save`, `delete`) let a guest through on every door, REST's `GET /:id`, `POST`, `PATCH` and `DELETE` included, and the admin UI's `can*` gating agreed. Postgres socket: the session had the flag from a JWT, but request frames wrote `false`, and the adapter sign-in never stored it (so its subscriptions lost it too). Also found: a Mongo socket with no session (`requireAuth: false`) was served by the base driver, which applies no rules, so it read every row and wrote anywhere. And the Postgres socket scoped the same caller as a guest on request frames only. Fixed with one identity per socket (`sessionUser` on Mongo, `sessionAuthContext` on Postgres), a no-session socket scoped as the anonymous user as REST scopes it, `isAnonymous` required on both sockets' session type, and the flag carried through the Mongo realtime service and `checkOperation`. REST's two middlewares were right and are now pinned. |
| Postgres in-process listener, fields it does not carry | **OPEN**, found while fixing the first fetch. `vectorSearch`, `page` and `withDeleted` are still not stored with the subscription. The socket refuses `vectorSearch` with `VECTOR_SEARCH_NOT_LIVE` and turns `page` into an offset; the in-process listener ignores both, so `.vectorSearch(…).listen()` in server code gets an ordinary listing. Mongo's in-process listener drops `vectorSearch` too. |
...
query, and `null`. Ten mutations each turned the tests written for them red: the identity not passed, passed
...
The guest flag is gated on both engines against a real database.
`packages/server-mongo/test/guest-identity-e2e.test.ts` runs mongod and the real
socket on a real HTTP server, with adapter and JWT sign-in and an
unauthenticated socket. It also covers in-process listeners and the scoped
driver calls REST makes, over a collection whose only rule is
`policy.registered()`. `packages/server-postgres/test/e2e/socket-guest-identity-e2e.test.ts`
runs a real Postgres as a superuser with `rebase_user`, JWT and adapter
sign-in, and a guests-only table for the socket with no session. Against the
unfixed code, 14 of the Mongo file's 18 cases failed: every guest and
anonymous case, plus one account case whose row a guest had just deleted. 7 of
the Postgres file's 10 failed. `test/policy-registered.test.ts` (common) pins
`checkOperation`, and `packages/server/test/rest-guest-identity.test.ts` pins
both REST middlewares. Fifteen mutations each turned a case red: the flag
dropped at each Mongo sign-in, in `sessionUser`, in the subscribe context, in
`handleClientMessage`, in `scopedDriver`, in `authContext()` and in
`checkOperation`; the base driver handed back to a socket with no session; the
flag dropped from the Postgres request frames, adapter sign-in and
`sessionAuthContext`; the socket with no session read as a guest again; and
each REST middleware dropping it.
