...
import { primaryKey, pgTable, integer, varchar, text, char, boolean, timestamp, date, time, jsonb, json, pgEnum, numeric, real, doublePrecision, bigint, serial, bigserial, pgPolicy, uuid, pgSchema, customType } from 'drizzle-orm/pg-core';

/** drizzle ships no `tsvector` builder; the search column round-trips as text. */
...
export const posts = pgTable("posts", {
    ...
    // Generated by Postgres from the `search` block on this collection.
    // Declared here so the query builder can find it; without this,
    // `.search()` silently falls back to the old ILIKE behaviour.
