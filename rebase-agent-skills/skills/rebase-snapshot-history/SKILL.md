---
name: rebase-snapshot-history
description: Guide for tracking and reverting snapshot changes with Rebase's built-in history/audit-log system. Use this skill when the user needs snapshot versioning, change tracking, audit logs, reverting to previous versions, or understanding how history snapshots work.
---

# Rebase Snapshot History

Rebase includes a built-in snapshot history system that records snapshots of every create, update, and delete operation. History entries are stored in a dedicated PostgreSQL table and exposed via REST API endpoints. The Studio admin panel provides a visual history tab with one-click revert.

> **IMPORTANT FOR AGENTS:** Snapshot history requires **two** things to be enabled: (1) the global `history` flag in the backend config, AND (2) `history: true` on each individual collection. If either is missing, no history is recorded for that collection.
...
await initializeRebaseBackend({
    ...
    // ── Enable snapshot history globally ──
    history: true,
...
|-----------|----------------|---------------|
| Create snapshot | `"create"` | Full snapshot values |
| Update snapshot | `"update"` | New values + previous values + list of changed fields |
| Delete snapshot | `"delete"` | Final snapshot values before deletion |
...
| `snapshot_id` | `string` | The ID of the snapshot this entry belongs to |
...
| `values` | `Record<string, unknown> \| null` | Full snapshot values snapshot at the time of the change |
| `previous_values` | `Record<string, unknown> \| null` | Previous snapshot values (only for updates) |
...
History endpoints are mounted under the data router at `{basePath}/data/:slug/:snapshotId/history`. They require the same authentication as regular data routes.
...
```
POST /api/data/:slug/:snapshotId/history/:historyId/revert
```

No request body is needed. The server fetches the stored `values` from the history entry and saves them through the normal snapshot save path.
...
| `400` | History entry belongs to a different snapshot (cross-snapshot revert blocked) |
...
```bash
# List history for snapshot 42 in the "products" collection
curl -H "Authorization: Bearer $TOKEN" \
     "http://localhost:3001/api/data/products/42/history?limit=10&offset=0"

# Revert snapshot 42 to the state captured in history entry "abc-123"
curl -X POST \
...
```typescript
async function getSnapshotHistory(slug: string, snapshotId: string, token: string) {
    const response = await fetch(
        `http://localhost:3001/api/data/${slug}/${snapshotId}/history?limit=20&offset=0`,
...
async function revertSnapshot(slug: string, snapshotId: string, historyId: string, token: string) {
    const response = await fetch(
        `http://localhost:3001/api/data/${slug}/${snapshotId}/history/${historyId}/revert`,
        {
            method: "POST",
            headers: { Authorization: `Bearer ${token}` },
        }
    ...
    const result = await response.json();
    // result.data — the reverted snapshot
...
CREATE TABLE IF NOT EXISTS rebase.snapshot_history (
    ...
    "values" JSONB,               -- full snapshot values snapshot
...
> **IMPORTANT FOR AGENTS:** The `rebase.snapshot_history` table is **auto-created** on server startup when history is enabled. You do NOT need to create it manually or include it in migrations. The `rebase` schema is also created automatically if it doesn't exist.
...
- **Snapshot IDs are stored as text** — regardless of the original column type.
- **Indexes** cover `(table_name, snapshot_id)` and `(table_name, snapshot_id, updated_at DESC)` for fast per-snapshot lookups.

## Retention and Automatic Pruning

The history service automatically prunes old entries to prevent unbounded storage growth. Pruning happens **non-blocking** after every insert (per-snapshot scope).

### Retention Configuration

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `maxEntries` | `number` | `200` | Maximum entries kept per snapshot. Oldest pruned first |
| `ttlDays` | `number` | `90` | Entries older than this many days are deleted |

### How Pruning Works

After every history insert, two pruning passes run in the background for that specific snapshot:
...
The `HistoryService` also exposes a `pruneExpired()` method for global TTL-based cleanup (deletes expired entries across ALL snapshots in one sweep). This is intended to be called periodically — e.g., from a cron job:
...
export default {
    ...
    handler: async () => {
        ...
        // This is an advanced pattern — the global prune is automatic per-snapshot
        console.log("History pruning handled automatically per-snapshot on insert");
    }
};
```
...
When history is enabled for a collection, Rebase Studio automatically adds a **History tab** in the snapshot edit view. This tab:
...
- For updates, the system fetches the **previous snapshot values** before saving (to compute `changed_fields` and store `previous_values`). This adds one extra `SELECT` query per update.
- Updates with zero actual changes are skipped entirely — no history entry is created.

### Storage Growth

- Each history entry stores **full JSONB snapshots** of both current and previous values. For snapshots with large text fields or complex nested structures, this can grow quickly.
- Default retention limits: **200 entries per snapshot** and **90 days TTL**. Adjust these based on your use case.
- All collections share the single `rebase.snapshot_history` table — monitor its size with:

```sql
SELECT pg_size_pretty(pg_total_relation_size('rebase.snapshot_history'));
```
