...
describe("renderJson", () => {
    it("emits exactly the ScanResult shape, with no colour and no extra keys", () => {
        ...
        expect(Object.keys(parsed).sort()).toEqual(
            ["database", "diagnostics", "exposedRoles", "findings", "platform", "scannedAt", "scannerIsPrivileged", "serverVersion", "stats"].sort()
        );
        // `exposedRoles` is part of the contract for the same reason: every
        // check gates on it, so a consumer cannot read `findings: []` without
        // knowing which roles were considered.
        expect(parsed.exposedRoles).toEqual(["PUBLIC", "anon", "authenticated"]);
