...
columnType: "bigint" } }
                ...
                // INTEGER (`primaryKeyColumnType`), so an int8 key would be
                // referenced by int4 foreign keys. `getSqlColumnType` has always
                // returned `INTEGER GENERATED BY DEFAULT AS IDENTITY` here and
                ...
                // method that exists, so the file did not compile at all.
