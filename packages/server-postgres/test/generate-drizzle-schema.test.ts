...
relationName: "author" }
        ...
        expect(cleanResult).toContain("authorId: text(\"author_id\").references((): AnyPgColumn => users.id, { onDelete: \"set null\" })");
...
relationName: "tags" }
        ...
        expect(cleanResult).toContain("post_id: text(\"post_id\").notNull().references((): AnyPgColumn => posts.id, { onDelete: \"cascade\" })");
        expect(cleanResult).toContain("tag_id: text(\"tag_id\").notNull().references((): AnyPgColumn => tags.id, { onDelete: \"cascade\" })");
...
columnType: "bigserial" }
            ...
            it("gives an increment id the same INTEGER identity the DDL gives it, whatever columnType says", async () => {
                const collections: CollectionConfig[] = [{
                    slug: "nums",
table: "nums",
name: "Nums",
...
columnType: "bigint" } }
                ...
                const result = await generateSchema(collections);
                // `isId: "increment"` is an identity column and its width is
                // INTEGER: every column that points at a numeric primary key is
                // INTEGER (`primaryKeyPgType`), so an int8 key would be
                // referenced by int4 foreign keys. The DDL generator has always
                // emitted `INTEGER GENERATED BY DEFAULT AS IDENTITY` here and
                // ignored `columnType`; this side honoured it, so the same
                // property was int8 in the file and int4 in the database — and
                // `bigserial(...).generatedByDefaultAsIdentity()` is not a
                // method that exists, so the file did not compile at all. One
                // plan, so there is one answer.
                expect(cleanSchema(result)).toContain("n_inc: integer(\"n_inc\").generatedByDefaultAsIdentity().primaryKey()");
                expect(cleanSchema(result)).not.toContain("bigint(\"n_inc\"");
            });
