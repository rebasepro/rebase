...
| `pnpm schema:generate` then `pnpm db:push` | The loop after every collection edit: collections → Drizzle schema → development database |
...
2. **Migrations are two steps**: `pnpm schema:generate`, then `pnpm db:push` in development or `pnpm db:generate && pnpm db:migrate` for production.
