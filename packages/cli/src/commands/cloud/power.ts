 ...
 */
import arg from "arg";
import { requireClient, requireProjectId, emit, confirmDestructive, success, reportError, type CloudClient } from "./context";
...
export async function powerCommand(action: PowerAction, rawArgs: string[]): Promise<void> {
    ...
    if (action !== "start") {
        await confirmDestructive({
            ...
            prompt: `${action === "stop" ? "Stop" : "Restart"} project ${projectId}? This causes downtime.`
        });
    }
    ...
    try {
        if (action === "stop") {
            ...
            emit(() => success(`Stopped project ${projectId}`), { success: true, projectId, status: "stopped" });
        } else if (action === "start") {
            await setStatus(client, projectId, "active");
            emit(() => success(`Started project ${projectId}`), { success: true, projectId, status: "active" });
        } else {
            await setStatus(client, projectId, "stopped");
            await new Promise((r) => setTimeout(r, 1500));
            await setStatus(client, projectId, "active");
            emit(() => success(`Restarted project ${projectId}`), { success: true, projectId, status: "active" });
        }
    } catch (e) {
