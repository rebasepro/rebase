...
export async function buildCommand(rawArgs: string[] = []): Promise<void> {
    ...
    for (const { name, app } of targets) {
        ...
        if (app.type === "backend") {
            ...
            /* The framework pins, read before the build and reported after it.
               Ranges no single version satisfies are refused by `buildBundle`
               itself, before it builds anything, for every command that builds
               a bundle. */
            ...
            /* Framework pins older than this CLI.
               ...
               over a guess would be worse than saying it out loud. (`drift` was
               read before the build, above.) */
