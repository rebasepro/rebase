...
export function PopupFormFieldInternal<M extends Record<string, unknown>>({
    ...
    const form = <div
        className={`text-surface-900 dark:text-white overflow-auto rounded-xs rounded-md bg-surface-card ${!open ? "hidden" : ""} cursor-grab max-w-[100vw]`}>
    ...
    const draggable = (
        <div
            ...
            className={`inline-block fixed z-20 shadow-outline rounded-md bg-surface-card ${!open ? "invisible" : "visible"
