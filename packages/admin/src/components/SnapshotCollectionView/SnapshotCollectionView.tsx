import type { AdditionalFieldDelegate, SnapshotAction, SnapshotCollection, Property } from "@rebasepro/types";
import {
    CollectionSize,
    Snapshot,
    SnapshotReference,
...
import { SnapshotCollectionRowActions, SnapshotCollectionTable } from "../SnapshotCollectionTable";
import { CollectionTableToolbar } from "../SnapshotCollectionTable/internal/CollectionTableToolbar";
...
import { SnapshotCollectionViewActions } from "./SnapshotCollectionViewActions";
import { SnapshotCollectionCardView } from "./SnapshotCollectionCardView";
import { SnapshotCollectionListView } from "./SnapshotCollectionListView";
import { SplitListView } from "./SplitListView";
import { SnapshotCollectionBoardView } from "./SnapshotCollectionBoardView";
...
import { getSubcollectionColumnId } from "../SnapshotCollectionTable/internal/common";
import { copySnapshotAction, deleteSnapshotAction, editSnapshotAction } from "../common/default_snapshot_actions";
import { PopupFormField } from "../SnapshotCollectionTable/internal/popup_field/PopupFormField";
import { GetPropertyForProps } from "../SnapshotCollectionTable/SnapshotCollectionTableProps";
import { DeleteSnapshotDialog } from "../DeleteSnapshotDialog";
import { useSelectionController } from "./useSelectionController";
import { SnapshotCollectionViewStartActions } from "./SnapshotCollectionViewStartActions";
 ...
 * @group Components
 */
export type SnapshotCollectionViewProps<M extends Record<string, unknown>> = {
    /**
...
} & SnapshotCollection<M>;

/**
 * This component is in charge of binding a driver path with an {@link SnapshotCollection}
 ...
 * of the lower level {@link SnapshotCollectionTable}
 *
 ...
 * can use {@link SnapshotCollectionTable}.
 *
 ...
 * @group Components
 */
const SnapshotCollectionViewInner = React.memo(
    function SnapshotCollectionViewInner<M extends Record<string, unknown>>({
        path: pathProp,
        ...
        const collection = useMemo(() => {
            ...
            return (userOverride ? mergeDeep(registryCollection, userOverride) : registryCollection) as SnapshotCollection<M>;
        ...
        }, [sideSnapshotController, openSnapshotMode, selectedSnapshotIdProp, path, urlController, analyticsController]);
        ...
        }, [path, sideSnapshotController, openSnapshotMode, urlController, unselectNavigatedSnapshot]);
...
parentSnapshotIds: parentSnapshotIds ?? EMPTY_ARRAY,
        ...
        const onCollectionModifiedForUser = useCallback((path: string, partialCollection: PartialSnapshotCollection<M>) => {
        ...
        const onColumnResize = useCallback(({
            ...
            const localCollection = buildPropertyWidthOverwrite(key, width) as PartialSnapshotCollection<M>;
        ...
        const onViewModeChange = useCallback((mode: ViewMode) => {
            ...
            if (userConfigPersistence) {
                onCollectionModifiedForUser(path, { defaultViewMode: mode } as PartialSnapshotCollection<M>);
            }
        ...
        const onKanbanPropertyChange = useCallback((property: string) => {
            ...
            if (userConfigPersistence) {
                onCollectionModifiedForUser(path, { kanbanColumnProperty: property } as PartialSnapshotCollection<M>);
            }
        ...
        const additionalFields = useMemo(() => {
            ...
            const subcollectionColumns: AdditionalFieldDelegate<M, any>[] = subcollectionsList.map((subcollection: SnapshotCollection) => {
                return {
        ...
        }, [collection, path, sideSnapshotController]);
...
parentSnapshotIds: parentSnapshotIds ?? EMPTY_ARRAY
        ...
        const onColumnsOrderChange = useCallback((newColumns: VirtualTableColumn[]) => {
            ...
            if (userConfigPersistence) {
                onCollectionModifiedForUser(path, { propertiesOrder: newPropertiesOrder } as PartialSnapshotCollection<M>);
            }
...
parentSnapshotIds,
        ...
        const ResolvedCollectionActions = useComponentOverride("Collection.Actions", SnapshotCollectionViewActions);
        const ResolvedCollectionTable = useComponentOverride("Collection.Table", SnapshotCollectionTable) as typeof SnapshotCollectionTable;
        ...
        const toolbarNode = (
            <CollectionTableToolbar
                ...
                actionsStart={<SnapshotCollectionViewStartActions
    ...
    }) as React.FunctionComponent<SnapshotCollectionViewProps<any>>;

export const SnapshotCollectionView = React.memo(
    function SnapshotCollectionView<M extends Record<string, unknown>>(props: SnapshotCollectionViewProps<M>) {
        const collectionRegistry = useCollectionRegistryController();
        const path = props.path ?? props.slug;
        const collection = collectionRegistry.getCollection(path) ?? props;

        const content = <SnapshotCollectionViewInner {...props} />;
...
function buildPropertyWidthOverwrite(key: string, width: number): PartialSnapshotCollection {
    if (key.includes(".")) {
        const [parentKey, ...childKey] = key.split(".");
        return { properties: { [parentKey]: buildPropertyWidthOverwrite(childKey.join("."), width) } } as PartialSnapshotCollection;
    }
    return { properties: { [key]: { ui: { columnWidth: width } } } } as PartialSnapshotCollection;
}
