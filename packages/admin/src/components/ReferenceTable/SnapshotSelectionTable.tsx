...
import { useSelectionController } from "../SnapshotCollectionView/useSelectionController";

import { useColumnIds } from "@rebasepro/core";
import { useSideDialogContext } from "../SideDialogs";
import { useAnalyticsController } from "@rebasepro/core";
import { useUrlController, useSideSnapshotController } from "../../index";

/**
 * @group Components
 */
export interface SnapshotSelectionProps<M extends Record<string, unknown>> {
 ...
 * You probably want to open this dialog as a side view using {@link useSnapshotSelectionTable}
 * @group Components
 */
export function SnapshotSelectionTable<M extends Record<string, unknown>>(
    props: SnapshotSelectionProps<M>
) {
    if (!props.collection) {
        return <ErrorView
            error={"Could not find collection"}/>
    }
    return <SnapshotSelectionTableInternal {...props} collection={props.collection} />;
}

function SnapshotSelectionTableInternal<M extends Record<string, unknown>>(
    {
        ...
        fixedFilter,
        maxSelection
    }: SnapshotSelectionProps<M> & { collection: SnapshotCollection<M> }) {
    ...
    const tableRowActionsBuilder = ({
        ...
        return <SnapshotCollectionRowActions
            width={width}
            frozen={frozen}
            snapshot={snapshot}
            size={size}
