...
import { SnapshotHistoryEntry } from "./SnapshotHistoryEntry";
 ...
 */
export function SnapshotHistoryView<M extends Record<string, unknown>>({
    snapshot,
    collection,
    formContext
}: SnapshotCustomViewParams<M>) {
    ...
    if (!snapshot) {
        return <div className="flex items-center justify-center h-full">
            <Label>HistoryIcon is only available for existing snapshots</Label>
        </div>;
    }
        ...
        <div className="flex flex-col gap-2 max-w-6xl mx-auto w-full">
            ...
            {!isLoading && entries.length === 0 && <>
                ...
                <Typography variant={"caption"} className={"ml-4"}>
                    When you save a snapshot, a new version is created and stored in the history.
                </Typography>
            </>}
                ...
                body={revertEntry
                    ? <div className="p-4">
                        <Typography variant={"caption"} color={"secondary"}>
                            This will save the snapshot with the values from{" "}
