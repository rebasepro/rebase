...
/**
 * Displays a single snapshot history revision entry.
 * Adapted from the snapshot_history plugin — now reads from backend API data.
 */
export function SnapshotHistoryEntry({
    actions,
    hover,
    collection,
    size,
    entry
}: SnapshotHistoryEntryProps) {
            ...
            <div className={"flex flex-col grow w-full m-1 shrink min-w-0"}>
                ...
                {(!changedFields || changedFields.length === 0) && (
                    <Typography variant={"caption"} color={"secondary"} className="ml-4">
                        {entry.action === "create" ? "Snapshot created" :
                            entry.action === "delete" ? "Snapshot deleted" : "No field changes recorded"}
                    </Typography>
                )}
