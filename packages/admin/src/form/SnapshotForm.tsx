...
import { Snapshot, SnapshotStatus, SnapshotValues } from "@rebasepro/types";
...
import { CustomFieldValidator, getSnapshotSchema } from "./validation";
import { SnapshotFormActions } from "./SnapshotFormActions";
...
/**
 * Headless snapshot form component.
 *
 * Renders a form for a snapshot collection without any CMS or backend dependencies.
 * All backend concerns (save, caching, analytics, plugin slots) are provided via
 * callback props. For CMS-connected usage, use {@link SnapshotFormBinding} instead.
 *
 * @group Components
 */
export function SnapshotForm<M extends Record<string, unknown>>({
    path,
    ...
    Builder,
    SnapshotFormActionsComponent = SnapshotFormActions,
    showDefaultActions = true,
    showSnapshotPath = true,
    ...
    const updateStatus = (status: SnapshotStatus) => {
        setStatus(status);
        onStatusChange?.(status);
    };

    const [valuesToBeSaved, setValuesToBeSaved] = useState<SnapshotValues<M> | undefined>(undefined);
    ...
    const [underlyingChanges] = useState<Partial<SnapshotValues<M>>>({});

    const initialSnapshotId: string | number | undefined = useMemo(() => {
        if (status === "new" || status === "copy") {
            return undefined;
        } else {
            return snapshotIdProp;
        }
    }, [snapshotIdProp, status]);

    const [snapshotId, setSnapshotId] = useState<string | number | undefined>(initialSnapshotId);
    const [snapshotIdError, setSnapshotIdError] = useState<boolean>(false);
    ...
    // Use externally computed initial values if provided, otherwise compute from snapshot + collection
    const baseInitialValues = useMemo(() => {
        if (computedInitialValues !== undefined) {
            return computedInitialValues;
        }
        // Fallback: compute from snapshot/collection (requires authController from context)
        // In headless mode without computedInitialValues, use snapshot values or defaults
        if ((status === "existing" || status === "copy") && snapshot) {
            return snapshot.values ?? getDefaultValuesFor(collection.properties);
        }
        return getDefaultValuesFor(collection.properties);
    }, [computedInitialValues, collection.properties, status, snapshot]);
    ...
    const onSubmit = (values: SnapshotValues<M>, formexController: FormexController<SnapshotValues<M>>) => {

        setSavingError(undefined);
        setSnapshotIdError(false);

        if (status === "existing") {
            if (!snapshot?.id) throw Error("Form misconfiguration when saving, no id for existing snapshot");
        } else if (status !== "new" && status !== "copy") {
            throw Error("New FormType added, check SnapshotForm");
        }

        if (!onSubmitProp) {
            console.warn("SnapshotForm: no onSubmit callback provided. Form submission has no effect.");
            ...
            ?.then((savedSnapshot) => {
                if (savedSnapshot) {
                    formexController.resetForm({
                        values: savedSnapshot.values || values,
                        submitCount: 0,
                        touched: {}
                    });
                }
    ...
    const internalFormex = useCreateFormex<M>({
        ...
        initialDirty,
        debugId: `SnapshotForm:${path}/${snapshotIdProp}`,
    ...
    const afterSave = (updatedSnapshot: Snapshot<M>) => {
        onResetProp?.();
    ...
    const lastSavedValues = useRef<SnapshotValues<M> | undefined>(snapshot?.values);
    const save = async (values: SnapshotValues<M>): Promise<Snapshot<M> | void> => {
        const valuesToSave = status === "existing"
            ? getChanges(values, snapshot?.values || {}) as SnapshotValues<M>
            : values;

        if (status === "existing" && Object.keys(valuesToSave).length === 0 && snapshot) {
        ...
        return Promise.resolve(onSubmitProp(valuesToSave as M, formex))
            .then((result) => {
                if (result) {
                    afterSave(result as Snapshot<M>);
                }
                return result as Snapshot<M> | void;
            })
            .catch(e => {
                console.error(e);
                setSavingError(e);
    ...
    }, [formex.version, collection, snapshotId, path]);
    ...
    const validationSchema = useMemo(() => getSnapshotSchema(
        snapshotId,
        collection.properties,
        uniqueFieldValidator),
        [snapshotId, collection.properties, uniqueFieldValidator]);

    useOnAutoSave(autoSave, formex, lastSavedValues, save);

    useEffect(() => {
        if (!autoSave && !formex.isSubmitting && underlyingChanges && snapshot) {
    ...
    }, [formex.isSubmitting, autoSave, underlyingChanges, snapshot, formex.values, formex.touched, formex.setFieldValue]);
    ...
    const formFields = () => {
        ...
        const isNewSnapshot = status === "new" || status === "copy";
            ...
            <FormLayout>
                {formFieldKeys.map((key) => {
                    ...
                    if (property) {
                        ...
                        const shouldAutoFocus = isNewSnapshot && !disabled && firstFocusableIndex === -1;
                    ...
                    if (additionalField && snapshot) {
                        ...
                        const child = AdditionalFieldBuilder
                            ? <AdditionalFieldBuilder snapshot={snapshot} context={additionalFieldContext}/>
    ...
    const formView = <ErrorBoundary>
        <>
            ...
            {!Builder && <div className={"w-full flex flex-col items-start my-4 lg:my-6"}>
                ...
                {!snapshot?.values && initialStatus === "existing" &&
                    <Alert color={"warning"} size={"small"} outerClassName={"w-full mb-4 text-xs"}>
                        This snapshot does not exist yet
                    </Alert>}

                {showSnapshotPath && <Alert color={"base"} outerClassName={"w-full"} size={"small"}>
                    <code
                        className={"text-xs select-all text-text-secondary dark:text-text-secondary-dark"}>
                        {snapshot?.path ?? path}/{snapshotId}
                    </code>
                </Alert>}
            </div>}
            ...
            {initialSnapshotId && !snapshot && initialStatus !== "new" && <Alert color={"info"} size={"small"}>
                This snapshot does not exist yet
            </Alert>}
    ...
    useEffect(() => {
        if (snapshotId && onIdChange)
            onIdChange(snapshotId);
    }, [snapshotId, onIdChange]);

    if (!collection || !path) {
        throw Error("INTERNAL: Collection and path must be defined in form context");
    }

    const SnapshotFormActionsRender = SnapshotFormActionsComponent as React.FC<SnapshotFormActionsProps>;

    const dialogActions = <SnapshotFormActionsRender
        collection={collection}
        path={path}
        snapshot={snapshot}
...
function useOnAutoSave<M extends Record<string, unknown>>(autoSave: undefined | boolean, formex: FormexController<M>, lastSavedValues: React.MutableRefObject<SnapshotValues<M> | undefined>, save: (values: SnapshotValues<M>) => Promise<Snapshot<M> | void>) {
    useEffect(() => {
        if (!autoSave) return;
