...
author_id: 5 }
            ...
            const result = await dataService.fetchCollection("authors/5/posts", {
                searchString: "mental",
                limit: 25
            });
...
post_id: 123 }
            ...
            const result = await dataService.fetchCollection("posts/123/comments", {
                searchString: "mental",
                limit: 20
            });
...
tag_id: 19 }
            ...
            // searchRows calls fetchRowsWithConditions which doesn't handle subcollection paths
            // It would need to be called with just the base collection path
            const result = await dataService.searchRows("posts", "mental", {
                limit: 30
            });
    ...
    describe("fetchRelatedSnapshots with search", () => {
        it("should pass search parameters correctly to fetchSnapshotsUsingJoins", async () => {
            const mockResults = [
                { id: 1,
...
tag_id: 19 }
            ...
            // Test fetchRelatedSnapshots directly with search
            const result = await dataService.fetchRelatedSnapshots("tags", 19, "posts", {
                searchString: "mental",
                limit: 20
            });
