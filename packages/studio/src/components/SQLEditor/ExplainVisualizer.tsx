...
export const ExplainVisualizer: React.FC<ExplainVisualizerProps> = ({ plan, isRoot = true }) => {
    ...
    return (
        <div className={cls("flex flex-col", isRoot ? "p-4" : "pl-6 mt-2 relative")}>
            {/* Tree branch line */}
            {!isRoot && (
                <div className="absolute left-2.5 top-0 bottom-0 w-px bg-hairline -z-10"></div>
            )}
            {!isRoot && (
                <div className="absolute left-2.5 top-5 w-3 h-px bg-hairline -z-10"></div>
            )}

            <div className={cls("border rounded-md bg-surface-card text-text-primary dark:text-text-primary-dark shadow-xs relative z-10 w-[420px] max-w-full", defaultBorderMixin)}>
                <div
                ...
                >
                    <div className="flex items-center space-x-2">
                        ...
                        {plan["Relation Name"] && (
                            <span className="font-mono text-[11px] text-text-secondary dark:text-text-secondary-dark px-2 py-0.5 rounded bg-hairline ml-2">
                ...
                <Collapse in={expanded}>
                    <div className="px-4 py-3 border-t border-hairline text-[13px] flex flex-col gap-2">
                        ...
                        {plan.Filter && (
                            <div className="mt-1">
                                ...
                                <code className="block w-full p-2 bg-surface-well border border-hairline rounded font-mono text-[12px] truncate">{plan.Filter}</code>
                            </div>
                        )}
                        {plan["Index Cond"] && (
                            <div className="mt-1">
                                <span className="text-text-disabled dark:text-text-disabled-dark block mb-1">Index Cond:</span>
                                <code className="block w-full p-2 bg-surface-well border border-hairline rounded font-mono text-[12px] truncate">{plan["Index Cond"]}</code>
                            </div>
                        )}
                        {plan["Hash Cond"] && (
                            <div className="mt-1">
                                <span className="text-text-disabled dark:text-text-disabled-dark block mb-1">Hash Cond:</span>
                                <code className="block w-full p-2 bg-surface-well border border-hairline rounded font-mono text-[12px] truncate">{plan["Hash Cond"]}</code>
                            </div>
                        )}
                    </div>
                </Collapse>
