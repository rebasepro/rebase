import { classifyStorageFailure } from "../src/components/StorageView/storage-failure";
...
describe("classifyStorageFailure", () => {
    it("reads a 403 as the project's own policy, not a platform failure", () => {
        ...
        const f = classifyStorageFailure(err);
    ...
    it("classifies the message even when the SDK dropped the status", () => {
        ...
        expect(classifyStorageFailure(new Error("Not authorized for this object")).kind).toBe("denied");
        expect(classifyStorageFailure(new Error("Forbidden")).kind).toBe("denied");
    });

    it("treats a 401 the same way — it is the same conversation", () => {
        expect(classifyStorageFailure(Object.assign(new Error("no"), { status: 401 })).kind).toBe("denied");
    });

    it("still reports a real failure as a real failure", () => {
        const f = classifyStorageFailure(new Error("network request failed"));
        expect(f.kind).toBe("unavailable");
        expect(f.retryable).toBe(true);
    });

    it("does not read a 500 as a refusal", () => {
        const f = classifyStorageFailure(Object.assign(new Error("boom"), { status: 500 }));
        expect(f.kind).toBe("unavailable");
    });

    it("never re-words the server", () => {
        // The console and the CLI must not say different things about one refusal.
        expect(classifyStorageFailure(new Error("Not authorized for this object")).detail)
            .toBe("Not authorized for this object");
    });

    it("survives something that is not an Error at all", () => {
        expect(classifyStorageFailure("nope").kind).toBe("unavailable");
        expect(classifyStorageFailure(null).kind).toBe("unavailable");
    });
});
