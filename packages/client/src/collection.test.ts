...
describe("createCollectionClient", () => {
    ...
    describe("count()", () => {
        it("should call the /count endpoint and return the count", async () => {
            (transport.request as ReturnType<typeof vi.fn>).mockResolvedValue({ count: 42 });
        ...
        it("should forward where filters to the count endpoint", async () => {
            (transport.request as ReturnType<typeof vi.fn>).mockResolvedValue({ count: 7 });
            ...
            const calledUrl = (transport.request as ReturnType<typeof vi.fn>).mock.calls[0][0] as string;
            expect(calledUrl).toContain("status=");
            expect(result).toBe(7);
        });

        it("should forward orderBy to the count endpoint", async () => {
            (transport.request as ReturnType<typeof vi.fn>).mockResolvedValue({ count: 15 });
        ...
        it("should NOT include limit or offset in the count request", async () => {
            (transport.request as ReturnType<typeof vi.fn>).mockResolvedValue({ count: 100 });
        ...
        it("should forward searchString to the count endpoint", async () => {
            (transport.request as ReturnType<typeof vi.fn>).mockResolvedValue({ count: 3 });
        ...
        it("should forward multiple where filters correctly", async () => {
            (transport.request as ReturnType<typeof vi.fn>).mockResolvedValue({ count: 5 });
