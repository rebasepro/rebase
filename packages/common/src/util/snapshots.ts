...
export function getDefaultValuesFor<M extends Record<string, unknown>>(properties: Properties): Partial<SnapshotValues<M>> {
...
/**
 * Update the automatic values in a snapshot before save
 * @group Driver
 */
export function updateDateAutoValues<M extends Record<string, unknown>>({
    ...
    {
        inputValues: Partial<SnapshotValues<M>>,
...
/**
 * Add missing required fields, expected in the collection, to the values of a snapshot
 * @param values
 * @param properties
 * @group Driver
 */
...
export function getReferenceFrom<M extends Record<string, unknown>>(snapshot: Snapshot<M>): SnapshotReference {
...
export function getRelationFrom<M extends Record<string, unknown>>(snapshot: Snapshot<M>): SnapshotRelation {
    return new SnapshotRelation(snapshot.id, snapshot.path, snapshot as unknown as Record<string, unknown>);
}

/**
 * Normalize a value into a proper SnapshotRelation instance.
 * Handles SnapshotRelation class instances, and plain objects
 * with `__type === "relation"` or an `isSnapshotRelation()` method.
 *
 ...
 */
export function normalizeToSnapshotRelation(value: unknown, propertyType?: string): SnapshotRelation | null {
    if (value instanceof SnapshotRelation) return value;
    ...
    const isRelationLike =
        ...
        (typeof obj.isSnapshotRelation === "function" && (obj.isSnapshotRelation as () => boolean)()) ||
        (typeof obj.isSnapshotReference === "function" && (obj.isSnapshotReference as () => boolean)()) ||
...
export function traverseValuesProperties<M extends Record<string, unknown>>(
    ...
    const updatedValues = Object.entries(properties)
        ...
        .reduce((a, b) => ({ ...a,
...b }), {}) as SnapshotValues<M>;
    // Use mergeDeep to preserve class instances like SnapshotReference, GeoPoint
...
/**
 * Create a hydrated relation reference that includes the full snapshot data.
 * Used when snapshot data has been pre-fetched (e.g., via batch loading or JOINs).
 */
export function createRelationRefWithData(id: string | number, path: string, data: Snapshot): RelationRefWithData {
    return { id,
path,
__type: "relation",
data };
