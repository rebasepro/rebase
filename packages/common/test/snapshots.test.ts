...
autoValue: "on_update" } as Property
    ...
    it("sets on_create and on_update for new snapshots", () => {
    ...
    it("sets on_update but not on_create for existing snapshots", () => {
...
describe("getReferenceFrom", () => {
    it("creates a SnapshotReference from a snapshot", () => {
        const snapshot = { id: "abc123",
path: "users",
values: { name: "Test" } };
        const ref = getReferenceFrom(snapshot as any);
        expect(ref).toBeInstanceOf(SnapshotReference);
...
path: "users",
values: {} };
        expect(() => getReferenceFrom(snapshot as any)).toThrow("Only string IDs");
    });
});
