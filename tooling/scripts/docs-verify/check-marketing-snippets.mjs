 ...
 *
 * The check strips the markup back to source and then applies three tests:
 *
 ...
 *   3. Every `rebase <cmd> <sub>` shown must dispatch. The command tree comes
 *      from `cli-commands.mjs`, which is derived from the CLI source; it lived
 *      here until the agent skills and the example READMEs needed the same
 *      thing (`check-doc-commands.mjs`).
 */
