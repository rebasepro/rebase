 ...
 * ## What counts
 *
 * A line that contains the exact English value of a declared key — quoted,
 * between two tags, or as the whole line, which is where JSX text sits once it
 * wraps — and does not itself call `t(`. The second half matters: `t("fix_errors") ?? "Fix errors"`
...
export function translatedStrings(source = fs.readFileSync(EN, "utf8")) {
...
/**
 * The key whose English value `line` writes out untranslated, or `null`.
 *
 * @param {string} line a source line
 * @param {{ key: string, value: string }[]} strings from {@link translatedStrings}
 */
export function untranslatedKeyOn(line, strings) {
    ...
    if (/\bconsole\.(?:log|info|warn|error|debug|trace)\s*\(/.test(line)) return null;
    ...
    if (/^\s*(group|icon):\s*"/.test(trimmed) || /^(group|icon):\s*"/.test(trimmed)) return null;
    // JSX text on a line of its own counts too: Prettier moves text that does
    // not fit between the tags onto its own line, and a check that only saw
    // `"value"` and `>value<` missed every wrapped one — 37 when it was added.
    for (const { key, value } of strings) {
        if (line.includes(`"${value}"`) || line.includes(`>${value}<`) || trimmed === value) return key;
    }
    return null;
}
...
function main() {
    ...
    for (const file of SCANNED.flatMap(sourceFiles)) {
        ...
        for (const line of fs.readFileSync(file, "utf8").split("\n")) {
            const key = untranslatedKeyOn(line, strings);
