...
So the sync compiles them: `tooling/design-sync/tailwind-entry.css` →
...
- Regenerate with `bash tooling/design-sync/rebuild.sh` (or `--css-only` to skip the
...
- **`.ds-sync/` is a toolchain, not a cache.** It is gitignored and it sits next
  ...
  `bash tooling/design-sync/rebuild.sh --css-only`.
- `ds-bundle/` **is** regenerable output — the DesignSync package build writes it,
  ...
  it pairs them with (`tooling/design-sync/previews/`) are both tracked.
...
CSS. The harness is committed under `tooling/design-sync/audit/`. **Re-run both after
