...
#
# Usage:
#   ./scripts/validate-no-workspace-protocol.sh          # pack + check
#   ./scripts/validate-no-workspace-protocol.sh --quick   # check source only (no pack)
#
# Exit codes:
#   0 = clean
...
if $QUICK; then
  ...
  for pkg_json in packages/*/package.json rebase-agent-skills/package.json; do
    [ -f "$pkg_json" ] || continue
