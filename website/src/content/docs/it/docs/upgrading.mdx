...
Sei passaggi, a partire dal più recente. Inizia da quello immediatamente superiore alla versione che stai utilizzando e procedi verso il basso — oppure, se provieni dalla versione 0.12, inizia dal fondo e procedi verso l'alto.

| Sei sulla | Leggi | Cosa comporta per te |
|---|---|---|
| **0.21 – 0.22** | [0.21 → next](/docs/upgrading/0-21-to-next/) | Non ancora rilasciato: ciò che è stato unito su `main` ed è già descritto in questa documentazione. Una chiave API su un percorso annidato richiede `read` sui parent, variabili numeriche e durate di sessione illeggibili impediscono l'avvio, un repository di autenticazione personalizzato richiede `unlinkUserIdentity`, e trasformare un ospite in account richiede il captcha `register`. 0.22 non dichiara modifiche incompatibili, quindi è un solo passaggio da 0.21. |
