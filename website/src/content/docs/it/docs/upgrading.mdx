...
|---|---|---|
| **0.18 – 0.20** | [0.18 → next](/docs/upgrading/0-18-to-next/) | Non ancora rilasciato: cosa è integrato su `main` e già descritto in questa documentazione. `context.client.data` non compila più all'interno di una callback di collection — esegui le query tramite `context.data`. Le versioni 0.19 e 0.20 non dichiarano breaking change, quindi questo è un singolo passaggio a partire da 0.18. |
