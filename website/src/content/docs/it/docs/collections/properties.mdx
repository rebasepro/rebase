...
### Configurazione UI (`ui`)

Le opzioni relative all'interfaccia utente sono raggruppate sotto il sotto-oggetto `ui`:
...
| Proprietà | Tipo | Descrizione |
|----------|------|-------------|
| `ui.readOnly` | `boolean` | Impedisce la modifica |
| `ui.disabled` | `boolean \| PropertyDisabledConfig` | Disabilita con tooltip opzionale |
| `ui.hideFromCollection` | `boolean` | Nasconde dalla vista tabella |
| `ui.columnWidth` | `number` | Larghezza della colonna in pixel (vista tabella) |
| `ui.widthPercentage` | `number` | Larghezza del campo come percentuale nella vista modulo |
| `ui.Field` | `React.ComponentType` | Componente campo personalizzato |
| `ui.Preview` | `React.ComponentType` | Componente cella di tabella personalizzato |
