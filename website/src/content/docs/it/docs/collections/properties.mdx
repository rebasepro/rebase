...
| `columnType` | `"varchar" \| "text" \| "char" \| "uuid"` | Colonna del database: `"varchar" \| "text" \| "char" \| "uuid"` |
...
| Proprietà | Tipo | Descrizione |
|----------|------|-------------|
| `enum` | `EnumValues` | Renderizza come selezione con valori numerici. Unlike a string enum this creates **no Postgres enum type** — the column stays `NUMERIC` or `INTEGER`, so the panel offers the values and the database does not enforce them |
