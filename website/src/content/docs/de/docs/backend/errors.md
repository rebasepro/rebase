...
| `INVALID_AGGREGATE_WINDOW` | 400 | `offset`, `page` oder `orderBy` bei einem Aggregat ohne `groupBy`, oder ein Cursor bei irgendeinem Aggregat. Ein ungruppiertes Aggregat ist eine Zeile, und eine Gruppe ist keine Zeile, nach der ein Cursor fortsetzen kann. | Fügen Sie `groupBy` hinzu oder lassen Sie sie weg; paginieren Sie Gruppen mit `offset` oder `page`. Siehe [Aggregate](/docs/sdk/aggregates-and-search/#aggregates). |
...
| `INVALID_CONFLICT_TARGET` | 400 | Das `on_conflict` / `onConflict` eines Upserts benennt Spalten ohne Eindeutigkeitsgarantie, benennt sie ohne `upsert: true` oder wird bei einem verschachtelten Create gesendet, wo eine getroffene Zeile unter den neuen Parent wandern würde. Postgres würde andernfalls mit 42P10 aus einer Transaktion heraus antworten, die bereits Arbeit verrichtet hat. | Deklarieren Sie `validation: { unique: true }` oder einen `unique`-Index; die Nachricht listet die vorhandenen Targets auf. |
| `INVALID_CURSOR` | 400 | Der `after`-Cursor ist nicht lesbar oder nennt eine Zeilen-ID, die nicht zu den Schlüsselspalten der Collection passt: Er wurde gekürzt, neu kodiert oder von Hand gebaut. | Geben Sie `meta.nextCursor` der vorherigen Seite unverändert zurück. Siehe [Paginierung](/docs/sdk/pagination/). |
...
| `UNKNOWN_RELATION` | 400 / 404 | `?include=` benennt etwas, das keine Relation in der Collection ist. Derselbe Code antwortet mit **404**, wenn stattdessen ein geschachtelter *URL-Pfad* eine solche benennt, z. B. `/api/data/authors/1/posts`, wo `authors` keine deklariert – dort benennt die URL nichts, weshalb es sich um ein Not-Found und nicht um einen fehlerhaften Request handelt. | Überprüfen Sie den Namen der Relation – die Nachricht listet die der Collection auf. Eine Rückwärtsreferenz muss auf dem Parent deklariert sein, um traversiert werden zu können. |
...
| `VECTOR_CURSOR_UNSUPPORTED` | 400 | Ein Cursor (`after` / `startAfter`) wurde mit einer Vektorsuche kombiniert. Die Zeilen sind nach einer pro Abfrage berechneten Distanz sortiert, die keinen Cursor tragen kann, daher hat eine Vektor-Auflistung kein `nextCursor`. | Paginieren Sie die nächsten Zeilen mit `limit`/`offset`. |
...
| `STORAGE_NOT_CONFIGURED` | 501 / 503 | Auf diesem Server ist kein Storage-Backend konfiguriert, oder keines ist der Standard und der Request nennt keine Storage-Quelle; die Nachricht nennt die bedienten Quellen (501). Ein fortsetzbarer Upload, dessen Quelle vor dem Abschluss verschwunden ist, ergibt 503. | Konfigurieren Sie S3, GCS oder lokalen Speicher, oder nennen Sie eine Quelle mit `?storageId=`. |
...
| `UNKNOWN_STORAGE_SOURCE` | 400 / 404 | Der Request hat eine Storage-Quelle (`?storageId=`) benannt, die dieses Projekt nicht deklariert. Ein `bucket`, den dieses Deployment nicht bedient, ist derselbe Code mit **404** (bei einer Auflistung sowie bei einem Upload, Ordner oder fortsetzbaren Upload auf S3 oder GCS) – der Speicher ist das, was fehlt, und `details` benennt die Buckets und Quellen, die existieren. Beides wurde früher als „file not found“ zurückgegeben, identisch mit einem Schlüssel, der schlicht nicht existiert. | Deklarieren Sie die Quelle in `config/resources.ts` oder prüfen Sie `GET /api/storage/sources`. |
...
`pnpm verify:docs` schlägt fehl, wenn ein Code, den der Server auslösen kann, in diesen Tabellen fehlt, wenn eine Tabelle einen Code auflistet, den nichts auslösen kann, wenn die Status einer Zeile nicht genau die sind, mit denen der Quellcode den Code auslöst oder wenn eine Code-Familie wie `PG_<SQLSTATE>` keine Zeile für einen SQLSTATE enthält, dem Aufrufer begegnen. Der entsprechende Schritt ist `tooling/scripts/docs-verify/check-error-codes.mjs`.
