...
`init` generiert eine sofort einsatzbereite `.env` für Sie (Geheimnisse, Datenbank-Passwort, ein freier lokaler Port) und bietet an, die Abhängigkeiten zu installieren — kein Bearbeiten nötig, um loszulegen. Dann:

```bash
pnpm dev                  # Datenbank, Schema, Backend + Frontend
```

Das ist der gesamte erste Lauf. `rebase dev` startet eine verwaltete PostgreSQL für das Projekt, generiert das Datenbankschema aus Ihren Sammlungen, wendet es an und liefert aus — kein Docker, und sonst nichts zu installieren.

Es gibt die beiden gebundenen URLs aus. Diese Ports werden aus dem Projektpfad abgeleitet, damit mehrere Rebase-Projekte parallel laufen können — verwenden Sie also die URLs in Ihrem Terminal statt `localhost:5173`. Das erste Konto, das Sie registrieren, wird zum Administrator.
