...
| `table` | `string` | PostgreSQL-Tabellenname. Standardmäßig `toSnakeCase(slug)` — nur setzen, um die URL vom Tabellennamen zu entkoppeln. |
| `admin.icon` | `string` | Name eines [Lucide](https://lucide.dev/icons)-Icons, z. B. `"FileText"`, `"ShoppingCart"`. |
...
|----------|------|---------|-------------|
| `defaultViewMode` | `"list" \| "table" \| "cards" \| "kanban"` | `"list"` | Standardansichtsmodus |
