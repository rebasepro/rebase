...
UI-bezogene Optionen werden im `ui`-Unterobjekt verschachtelt:
...
| Property | Type | Beschreibung |
|----------|------|-------------|
| `ui.readOnly` | `boolean` | Bearbeitung verhindern |
| `ui.disabled` | `boolean \| PropertyDisabledConfig` | Deaktivieren mit optionalem Tooltip |
| `ui.hideFromCollection` | `boolean` | Aus der Tabellenansicht ausblenden |
| `ui.columnWidth` | `number` | Spaltenbreite in Pixeln (Tabellenansicht) |
| `ui.widthPercentage` | `number` | Feldbreite als Prozentwert in der Formularansicht |
| `ui.Field` | `React.ComponentType` | Benutzerdefinierte Feldkomponente |
| `ui.Preview` | `React.ComponentType` | Benutzerdefinierte Tabellenzellenkomponente |
