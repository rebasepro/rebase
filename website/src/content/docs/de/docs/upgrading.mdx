...
Sechs Schritte, die neuesten zuerst. Beginnen Sie mit der Version, die direkt über Ihrer aktuellen liegt, und arbeiten Sie sich nach unten vor – oder fangen Sie, wenn Sie von Version 0.12 kommen, ganz unten an und arbeiten Sie sich nach oben vor.

| Sie sind auf | Lesen | Was sich für Sie ändert |
|---|---|---|
| **0.21 – 0.22** | [0.21 → next](/docs/upgrading/0-21-to-next/) | Noch nicht veröffentlicht: was auf `main` gemergt und in dieser Dokumentation bereits beschrieben ist. Ein API-Schlüssel auf einem verschachtelten Pfad braucht `read` auf den Parents, unlesbare numerische Variablen und Session-Lebensdauern verhindern den Start, ein eigenes Auth-Repository braucht `unlinkUserIdentity`, und Gast-Upgrades verlangen die `register`-Captcha. 0.22 deklariert keinen Breaking Change, daher ist dies ein Schritt ab 0.21. |
