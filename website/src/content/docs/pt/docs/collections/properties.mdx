...
### Configuração de UI (`ui`)

As opções relacionadas à UI são aninhadas sob o sub-objeto `ui`:
...
| Propriedade | Tipo | Descrição |
|----------|------|-------------|
| `ui.readOnly` | `boolean` | Impedir edição |
| `ui.disabled` | `boolean \| PropertyDisabledConfig` | Desabilitar com tooltip opcional |
| `ui.hideFromCollection` | `boolean` | Ocultar da visualização de tabela |
| `ui.columnWidth` | `number` | Largura da coluna em pixels (visualização de tabela) |
| `ui.widthPercentage` | `number` | Largura do campo como porcentagem na visualização de formulário |
| `ui.Field` | `React.ComponentType` | Componente de campo personalizado |
| `ui.Preview` | `React.ComponentType` | Componente de célula de tabela personalizado |
