...
|---|---|---|
| **0.18 – 0.20** | [0.18 → next](/docs/upgrading/0-18-to-next/) | Ainda não lançado: o que já foi integrado na `main` e descrito nesta documentação. `context.client.data` deixa de compilar em um callback de coleção — faça consultas por meio de `context.data`. As versões 0.19 e 0.20 não declaram breaking changes, portanto este é um único salto a partir da 0.18. |
