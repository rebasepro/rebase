...
description: Conecte o Claude Code, Cursor, Gemini CLI ou qualquer cliente MCP a um projeto Rebase — as 40 ferramentas que ele expõe, a credencial com a qual ele se autentica e a proteção de loopback que fica entre um agente e a produção.
---
...
Para o **Claude Code**, adicione-o ao `.mcp.json` na raiz do seu projeto:
...
O **Cursor** usa o mesmo formato em `.cursor/mcp.json`, e o **Gemini CLI** em
`.gemini/settings.json`. Qualquer cliente MCP que consiga iniciar um servidor stdio funciona —
o bloco acima é toda a integração.

`REBASE_PROJECT_DIR` deve ser o diretório que contém o `rebase.json`. Se você
omiti-lo, o servidor usará seu diretório de trabalho, que é o diretório no qual o cliente
o iniciou.
...
|---|---|---|
| `REBASE_PROJECT_DIR` | `process.cwd()` | Raiz do projeto — usada para encontrar coleções, `.env` e o estado do servidor de desenvolvimento |
| `REBASE_BASE_URL` | `http://localhost:3001` | URL do backend |
| `REBASE_API_TOKEN` / `REBASE_TOKEN` | *(vazio)* | O token usado para todas as chamadas de API |
| `REBASE_MCP_ALLOW_REMOTE_WRITES` | `false` | Isenta ferramentas destrutivas da proteção de loopback |

O servidor carrega o `.env` de `$REBASE_PROJECT_DIR/.env` ou
`$REBASE_PROJECT_DIR/app/.env` na inicialização.
...
- **Não protegidas — leituras:** `rebase_schema_introspect`, `rebase_doctor`,
...
40 ferramentas, em oito grupos. As ferramentas marcadas com ⚠ são recusadas contra destinos
não locais, a menos que você desative a proteção.

### Schema e banco de dados (11)
...
`rebase://schema` é listado **apenas se** o schema gerado estiver exatamente em
`app/backend/src/schema.generated.ts`. Esse é um caminho único pré-definido (*hardcoded*) sem
alternativas (*fallbacks*), portanto, um projeto estruturado de forma diferente — ou que ainda não executou
`rebase schema generate` — simplesmente não verá o recurso oferecido. Se ele estiver
ausente e você o esperava, verifique o caminho antes de concluir que o servidor está quebrado.
