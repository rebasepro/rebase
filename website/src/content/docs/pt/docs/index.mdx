...
O `init` gera um arquivo `.env` pronto para uso (segredos, senha do banco de dados, uma porta local livre) — sem necessidade de edição para começar. Inicie o contêiner PostgreSQL incluído, crie as tabelas e execute os servidores de desenvolvimento:

```bash
docker compose up -d db   # inicia o banco de dados
pnpm run db:push          # cria as tabelas a partir das suas coleções
pnpm dev                  # inicia o backend + frontend
```

Seu painel de administração está em `http://localhost:5173` e a API em `http://localhost:3001`. A primeira conta que você registrar torna-se a de administrador.
