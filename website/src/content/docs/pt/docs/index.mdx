...
Configure seu banco de dados em `.env`:
...
Se você estiver se conectando a um banco de dados existente, poderá fazer a introspecção para gerar automaticamente seus arquivos de coleção:
...
É isso. Seu painel de administração está em `http://localhost:5173` e a API em `http://localhost:3001`.

→ Siga o [guia de Início Rápido](/docs/getting-started/quickstart) completo para um passo a passo detalhado.
