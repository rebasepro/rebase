...
- **Bake it into an image** — `FROM rebasepro/server:0.19.0` then `COPY dist-bundle /bundle`, and deploy by changing a tag.
...
`REBASE_ADMIN_EMAIL` and `REBASE_ADMIN_PASSWORD` are new: on 0.17.3
