...
:::note[Cursor, Windsurf, Kiro and Copilot get one always-on file]
Those four load their whole rules directory into every request. A rule file per
skill meant about **84,000 characters** of Rebase reference in front of every
question a person asked, whether or not it was about Rebase — and an
instruction an assistant skims is one it does not follow.

They get `rebase.mdc` (or `rebase.md`) instead: a ~3 KB index with
`alwaysApply: true`, listing what each skill covers and the file to read. The
bodies sit in per-skill subdirectories and are opened on demand.
:::
...
`.github/copilot-instructions.md` into every scaffold, and most repositories have
a `.github/` for workflows. Install it with `--agent copilot`.
