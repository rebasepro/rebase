...
| `INVALID_OFFSET` | 400 | `?offset=` is not a whole number of 0 or more. | Send a non-negative integer. |
| `INVALID_ORDER_BY` | 400 | `?orderBy=` is not `field`, `field:desc`, or a JSON array of `{ field, direction }`. | See the message; it shows all three spellings. |
| `INVALID_PAGE` | 400 | `?page=` is not a whole number of 1 or more. Pages are 1-based, so `?page=0` is a mistake rather than the first page. | Send `1` or higher, or use `?offset=`. |
| `INVALID_PARAM` | 400 | A query parameter is malformed. | See the message. |
| `INVALID_VECTOR` | 400 | `?vector=` is not a JSON array of numbers. | Send `[0.1,0.2,0.3]`. |
...
| `VALIDATION_EXCLUDED_FIELDS` | 400 | The body writes a column marked `excludeFromApi`. Those are the server's to set — a password hash, a verification token. | Remove the field. |
...
A write that Postgres rejects for a reason that is *the caller's data* answers
...
- **class 23**, integrity constraint violation: a duplicate, a foreign key that
  points at nothing, a NOT NULL column left empty;
...
| Code | Status | Means | Do |
| --- | --- | --- | --- |
| `PG_22P02` | 400 | A value could not be read as the column's type — the write-side twin of `INVALID_FILTER_VALUE`. | Send a value of the column's type. |
| `PG_23502` | 400 | A `NOT NULL` column was left empty. | Send the field, or give the column a default. |
| `PG_23503` | 400 | A foreign key points at a row that does not exist. | Create the target row first, or fix the id. |
| `PG_23505` | 409 | A unique constraint was violated. The message names the constraint. | Use a different value, or update the existing row. |
...
| `SCHEMA_CHANGE_FAILED` | 400 | Applying a planned schema change failed for a reason more specific codes do not cover. | See the message; it is the underlying failure verbatim. |
...
| `SCHEMA_EDIT_REFUSED` | 400 | The schema editor refused the edit. | See the message; it is the editor's own refusal. |
