...
| `reference` | Embedded reference to another snapshot | `varchar` (stores ID) |
...
| `defaultValue` | `any` | Default value for new snapshots |
...
References link to snapshots in another collection. They render as a preview card showing the referenced snapshot's details.
...
You can make fields dynamic so they react to the snapshot's values. There are two ways to do this:
