...
Six hops, newest first. Start at the one above the version you are on and work
downwards — or, if you are coming from 0.12, start at the bottom and work up.

| You are on | Read | What it does to you |
|---|---|---|
| **0.21 – 0.22** | [0.21 → next](/docs/upgrading/0-21-to-next/) | Not released yet: what is merged on `main` and already described in these docs. An API key on a nested path needs `read` on its parents, unreadable numeric variables and session lifetimes refuse to boot, a custom auth repository needs `unlinkUserIdentity`, and guest upgrades take the `register` captcha. 0.22 declares no breaking change, so this is one hop from 0.21. |
| **0.18 – 0.20** | [0.18 → 0.21](/docs/upgrading/0-18-to-0-21/) | `context.client.data` stops compiling in a collection callback — query through `context.data` — and `selectedEntities` gives way to a selection that can stand for rows nobody has loaded. 0.19 and 0.20 declare no breaking change, so this is one hop from 0.18. |
| **0.17** | [0.17 → 0.18](/docs/upgrading/0-17-to-0-18/) | Eleven breaking changes, most of them compile errors rather than silent behaviour changes. `rebase.data` disappears at runtime, peer ranges narrow to carets, and the Node floor moves to 22.22.0. |
