...
This release renames packages, changes what `id` means, drops CJS, moves the
admin panel to react-router 8, and — most importantly — **fixes three ways
access could be granted that you did not ask for**: a security rule that was
silently permissive, and two on the realtime socket. Read the first two sections
before anything else. They are the changes that alter who can read your data,
