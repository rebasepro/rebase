...
admin panel to react-router 8, and — most importantly — **fixes a security rule
that was silently permissive**. Read the first section before anything else: it
is the only change that can alter who can read your data, and it is the only one
...
## 5. `react-router` 8, and `react-router-dom` is gone
...
## 7. Every deprecated export is removed
...
## 8. `defaultSecurityRules` moved off the server config
...
[ ] grep for rebase.data — it is now rebase.dataAsAdmin (section 7)
[ ] grep for the other ten removed deprecated exports (section 7)
