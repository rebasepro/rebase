...
- **`setIn` could rewrite `Object.prototype`.** `setIn(values, path, value)` writes a form field at a dotted path, and nothing stopped the path naming the prototype chain: `__proto__.x`, `constructor.prototype.x` and `__proto__.0` all wrote onto `Object.prototype`, giving every object — and every array — a property the request chose. `res["__proto__"] = …` is a prototype setter rather than a property write, so the value left the object entirely. Paths here are property keys, and for a map property or a column mapped out of an imported CSV those are data. Both `@rebasepro/forms` and `@rebasepro/utils` refuse prototype-traversing paths now, in `setIn` and in `getIn` — handing back `Object.prototype` is how a polluted value is read out again. Marked breaking: `getIn(x, "constructor")` no longer returns a constructor. The Postgres transformer gained the same guard per key, where `JSON.parse` makes `__proto__` an *own* property that survives `hasOwnProperty` and replaces the row's prototype.

- **The in-process SDK dropped half of what a query asked for.** `context.data` and `client.data` take the same `FindParams`; the in-process accessor read four of its fields. `logical` — the `or(...)`/`and(...)` group — was never forwarded in `find`, `count` or `listen`, and a dropped filter does not fail, it widens: `data.posts.where(or(…)).find()` returned every row the caller's policies allowed. `page` was ignored, so every page served page one. `count()` ignored `searchString`. And `limit` reached the driver as `undefined`, which Postgres reads as *no LIMIT clause*, so an unbounded `find()` selected the whole table while reporting `meta.limit: 20`. One `resolveFindWindow` answers limit/offset/page for every transport now.

- **Realtime subscriptions ignored the offset and the filter they were given.** A live query's narrowing crosses four hops, each re-listing the fields it forwarded by hand, and `offset` and `logical` were missing from every list — accepted by four type-checked boundaries and discarded. A live list on page three served page one for as long as it stayed subscribed. The socket's de-duplication key was built from a subset of the props too, so two subscriptions differing only in `offset` or `logical` collided and the second listener was handed the first one's rows. The Mongo driver had the same gap and no implementation behind it; `or(...)`/`and(...)` and `skip` work there now.

- **An unreachable database was reported to callers as a bad request.** The create and update handlers stamped `BAD_REQUEST` onto any error that was not a `TypeError` or similar, gated on `isRebaseApiError` — whose entire body was `return error instanceof Error`. So a dropped connection, a pool timeout or a missing column after a skipped migration all answered 400, which tells the SDK not to retry and makes an outage count as user error. The driver classifies from the SQLSTATE it already holds: classes 22 and 23 are the caller's data, everything else is a 500.

- **A backend that was down looked exactly like being signed out.** `useAuthSubscription` caught every `getUser()` failure with "user just isn't logged in" — true of a 401, not of a 500, a CORS rejection or `Failed to fetch`. It declares an `authError` that `Rebase` renders a full-screen view for, and never set it, so that view was unreachable code and the app fell through to the login form.

- **The `Idempotency-Key` did not stop the duplicate it exists for.** The store recalled a key and, on a miss, wrote and then remembered it — so two tabs replaying one key both missed and both wrote. The key is claimed before the write now, in one statement, with a 409 for the loser and a release path so a failed write does not strand the key for a day.

- **Seven documented plugin slots rendered nothing.** `collection.filter-panel`, `dashboard.widget`, `entity.field.before`/`after`, `entity.row.actions`, `global.search` and `shell.toolbar` each have a props interface and a row in the slot reference in six languages, and nothing renders them. They are named in `UNRENDERED_SLOTS`, `Rebase` warns anyone who registers for one, the docs mark them, and a test derives the set by scanning for render sites so it cannot drift in either direction. `collection.error` was in the same state and is now rendered.

- **Smaller things found by using the panel.** The row count ignored the active search and cached across searches; searching a kanban board did nothing but make it flicker; `?fields=` did nothing on subcollection routes and stripped the key from any collection not keyed on `id`; a disabled table cell was still editable; `rebase auth reset-password` could reset an account you did not name, because it matched `ILIKE '%…%'` over email *or display name* and took the first row; a `.env` written with `export` read as empty.
