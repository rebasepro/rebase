...
`init` generates a ready-to-run `.env` for you (secrets, database password, a free local port) and offers to install dependencies — no editing needed to get started. Then:

```bash
pnpm dev                  # database, schema, backend + frontend
```

That is the whole first run. `rebase dev` starts a managed PostgreSQL for the project, generates the database schema from your collections, applies it, and serves — no Docker, and nothing else to install.

It prints the two URLs it bound. Those ports are derived from the project's path so that several Rebase projects can run side by side, so use the URLs in your terminal rather than `localhost:5173`. The first account you register becomes the admin.
