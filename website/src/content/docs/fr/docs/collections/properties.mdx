...
Les options liées à l'interface utilisateur sont imbriquées sous le sous-objet `ui` :
...
| Propriété | Type | Description |
|----------|------|-------------|
| `ui.readOnly` | `boolean` | Empêche l'édition |
| `ui.disabled` | `boolean \| PropertyDisabledConfig` | Désactiver avec info-bulle optionnelle |
| `ui.hideFromCollection` | `boolean` | Masquer de la vue tableau |
| `ui.columnWidth` | `number` | Largeur de colonne en pixels (vue tableau) |
| `ui.widthPercentage` | `number` | Largeur du champ en pourcentage dans la vue formulaire |
| `ui.Field` | `React.ComponentType` | Composant de champ personnalisé |
| `ui.Preview` | `React.ComponentType` | Composant de cellule de tableau personnalisé |
