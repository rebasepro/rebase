...
`init` génère pour vous un fichier `.env` prêt à l'emploi (secrets, mot de passe de base de données, un port local libre) — aucune modification n'est nécessaire pour commencer. Démarrez le conteneur PostgreSQL inclus, créez les tables, puis lancez les serveurs de développement :

```bash
docker compose up -d db   # démarrer la base de données
pnpm run db:push          # créer les tables à partir de vos collections
pnpm dev                  # démarrer le backend + le frontend
```

Votre panneau d'administration est disponible à l'adresse `http://localhost:5173` et l'API à `http://localhost:3001`. Le premier compte que vous enregistrez devient l'administrateur.
