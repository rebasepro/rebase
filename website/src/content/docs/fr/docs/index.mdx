...
`init` génère pour vous un fichier `.env` prêt à l'emploi (secrets, mot de passe de base de données, un port local libre) et propose d'installer les dépendances — aucune modification n'est nécessaire pour commencer. Ensuite :

```bash
pnpm dev                  # base de données, schéma, backend + frontend
```

C'est tout le premier lancement. `rebase dev` démarre un PostgreSQL géré pour le projet, génère le schéma de base de données à partir de vos collections, l'applique et sert — sans Docker, et sans rien d'autre à installer.

Il affiche les deux URLs qu'il a liées. Ces ports sont dérivés du chemin du projet pour que plusieurs projets Rebase puissent tourner en parallèle : utilisez donc les URLs de votre terminal plutôt que `localhost:5173`. Le premier compte que vous enregistrez devient l'administrateur.
