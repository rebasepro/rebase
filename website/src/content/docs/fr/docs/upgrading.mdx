...
Déjà en 0.17 ? Le saut vers 0.18 est publié et décrit ici : [0.17 → 0.18](/fr/docs/upgrading/0-17-to-0-18/).

En 0.18 ou plus récent ? Ce qui est déjà fusionné sur `main` mais pas encore publié est décrit ici : [0.18 → next](/fr/docs/upgrading/0-18-to-next/).
...
**Modifiez-les aussi.** Cette page les présentait comme orientés utilisateur et jamais obsolètes. Il s'agit du même singleton serveur, donc du même alias limité à l'administrateur :

- `context.client.data` dans un rappel (callback) d'entité → `context.data`, l'accesseur des requêtes dans les rappels. Il s'exécute avec le privilège de ce qui a déclenché le rappel, et `context.client.data` ne compile plus dans les versions actuelles.
- `client.data` dans un gestionnaire de cron → `client.dataAsAdmin` (depuis la 0.14, le contexte du gestionnaire le nomme `rebase`)

**N'y touchez pas :** `rebase.data` dans un SDK généré ou une application navigateur est un objet différent.
