...
Six étapes, de la plus récente à la plus ancienne. Commencez par celle qui se trouve juste au-dessus de votre version actuelle et descendez — ou, si vous partez de la version 0.12, commencez par le bas et remontez.

| Vous êtes en | À lire | Ce que cela implique |
|---|---|---|
| **0.21 – 0.22** | [0.21 → next](/docs/upgrading/0-21-to-next/) | Pas encore publié : ce qui est fusionné sur `main` et déjà décrit dans cette documentation. Une clé d'API sur un chemin imbriqué a besoin de `read` sur ses parents, les variables numériques et durées de session illisibles empêchent le démarrage, un dépôt d'authentification personnalisé a besoin de `unlinkUserIdentity`, et la conversion d'un invité en compte exige le captcha `register`. 0.22 ne déclare aucun changement incompatible : c'est donc une seule étape depuis 0.21. |
| **0.18 – 0.20** | [0.18 → 0.21](/docs/upgrading/0-18-to-0-21/) | `context.client.data` ne compile plus dans un callback de collection — effectuez vos requêtes via `context.data` — et `selectedEntities` cède la place à une sélection qui peut désigner des lignes que personne n'a chargées. Les versions 0.19 et 0.20 ne déclarent aucun changement cassant, il s'agit donc d'une seule étape depuis la 0.18. |
