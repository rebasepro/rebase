...
Pertinent uniquement si vous utilisez le panneau d'administration — `@rebasepro/admin`, `app`, `studio` ou `plugin-ai`. Une installation headless ne possède pas de routeur.
...
**`RebaseCMS` est désormais `RebaseAdmin`.** `mode: "cms"` dans `RebaseBackendConfig` reste inchangé — il décrit d'où proviennent les collections, pas l'interface utilisateur.
