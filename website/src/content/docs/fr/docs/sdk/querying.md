...
| `.find()` | Exécute la requête | Renvoie `FindResult<M>` |
...
> **Remarque :** Sans `.include("author")`, seul le champ scalaire `authorId` est renvoyé. L'objet `author` hydraté sera `undefined`.
