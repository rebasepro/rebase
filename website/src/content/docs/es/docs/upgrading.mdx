...
Seis saltos, de los más recientes a los más antiguos. Empieza en el que esté por encima de tu versión actual y avanza hacia abajo; o, si vienes de la versión 0.12, empieza desde abajo y avanza hacia arriba.

| Estás en | Leer | Qué efectos tiene |
|---|---|---|
| **0.21 – 0.22** | [0.21 → next](/docs/upgrading/0-21-to-next/) | Aún no publicado: lo que está fusionado en `main` y ya se describe en esta documentación. Una clave de API en una ruta anidada necesita `read` sobre sus padres, las variables numéricas y las duraciones de sesión ilegibles impiden el arranque, un repositorio de autenticación propio necesita `unlinkUserIdentity`, y convertir un invitado en cuenta exige el captcha de `register`. 0.22 no declara ningún cambio incompatible, así que es un solo salto desde 0.21. |
