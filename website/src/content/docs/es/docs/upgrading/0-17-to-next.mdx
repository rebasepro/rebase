...
## 26. Un proyecto con solo buckets nombrados nombra su predeterminado

`bucket("media")` a solas se promovía antes a bucket por defecto en el arranque,
con un aviso. Ahora el arranque rechaza cuando ningún bucket es el
predeterminado.

**Qué cambiar:** si todos los buckets de `config/resources.ts` están nombrados,
marca uno — `bucket("media", { default: true })` — o añade un
`bucket("(default)")` al lado. Un proyecto que ya declara uno por defecto no
necesita nada.
