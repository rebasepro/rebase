...
| `columnType` | `"varchar" \| "text" \| "char" \| "uuid"` | Columna de base de datos: `"varchar" \| "text" \| "char" \| "uuid"` |
...
| Propiedad | Tipo | Descripción |
|----------|------|-------------|
| `enum` | `EnumValues` | Renderizar como select con valores numéricos. Unlike a string enum this creates **no Postgres enum type** — the column stays `NUMERIC` or `INTEGER`, so the panel offers the values and the database does not enforce them |
