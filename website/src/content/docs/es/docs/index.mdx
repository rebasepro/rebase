...
`init` genera un archivo `.env` listo para usar (secretos, contraseña de base de datos, un puerto local libre) — no necesitas editar nada para empezar. Inicia el contenedor de PostgreSQL incluido, crea las tablas y luego ejecuta los servidores de desarrollo:

```bash
docker compose up -d db   # inicia la base de datos
pnpm run db:push          # crea las tablas a partir de tus colecciones
pnpm dev                  # inicia backend + frontend
```

Tu panel de administración se está ejecutando en `http://localhost:5173` y la API en `http://localhost:3001`. La primera cuenta que registres se convierte en administrador.
