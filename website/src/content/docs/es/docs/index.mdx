...
`init` genera un archivo `.env` listo para usar (secretos, contraseña de base de datos, un puerto local libre) y se ofrece a instalar las dependencias — no necesitas editar nada para empezar. Entonces:

```bash
pnpm dev                  # base de datos, esquema, backend + frontend
```

Esa es toda la primera ejecución. `rebase dev` arranca un PostgreSQL gestionado para el proyecto, genera el esquema de la base de datos a partir de tus colecciones, lo aplica y sirve — sin Docker y sin nada más que instalar.

Imprime los dos URLs a los que se asoció. Esos puertos se derivan de la ruta del proyecto para que varios proyectos Rebase puedan ejecutarse a la vez, así que usa los URLs de tu terminal en lugar de `localhost:5173`. La primera cuenta que registres se convierte en administrador.
