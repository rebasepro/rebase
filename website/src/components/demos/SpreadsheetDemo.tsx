...
export function SpreadsheetDemo() {
    ...
    return (
        <div className="w-full h-full min-h-[400px] bg-surface-frame text-surface-200 flex flex-col font-sans select-none pointer-events-none relative border border-hairline rounded-2xl overflow-hidden">
            {/* Toolbar */}
            <div className="h-12 border-b border-hairline bg-surface-sheet flex items-center px-4 gap-4 shrink-0">
                <div className="flex gap-2">
                    <div className="h-2 w-2 rounded-full bg-surface-raised-hover"></div>
                    <div className="h-2 w-2 rounded-full bg-surface-raised-hover"></div>
                    <div className="h-2 w-2 rounded-full bg-surface-raised-hover"></div>
                </div>
                <div className="h-4 w-px bg-surface-raised-hover"></div>
            ...
            {/* Table */}
            <div className="flex-1 overflow-hidden bg-surface-frame flex flex-col">
                <div className="grid grid-cols-[48px_minmax(150px,1fr)_minmax(200px,2fr)_120px_150px] border-b border-hairline text-xs font-semibold text-surface-400 bg-surface-field">
                    <div className="p-3 border-r border-hairline flex items-center justify-center">
                        <div className="w-3.5 h-3.5 rounded-sm border border-hairline-strong"></div>
                    </div>
                    <div className="p-3 border-r border-hairline flex items-center gap-2">Name</div>
                    <div className="p-3 border-r border-hairline flex items-center gap-2">Email</div>
                    <div className="p-3 border-r border-hairline flex items-center gap-2">Status</div>
                ...
                <div className="flex flex-col">
                    {users.map((user, idx) => {
                        ...
                        return (
                            <div key={user.id} className="grid grid-cols-[48px_minmax(150px,1fr)_minmax(200px,2fr)_120px_150px] border-b border-hairline text-sm text-surface-300 hover:bg-surface-hover transition-colors">
                                <div className="p-3 border-r border-hairline flex items-center justify-center">
                                    <span className="text-xs text-surface-600">{user.id}</span>
                                </div>
                                <div className="p-3 border-r border-hairline flex items-center truncate">
                                    {user.name}
                                </div>
                                <div className="p-3 border-r border-hairline flex items-center text-surface-400 truncate">
                                    {user.email}
                                </div>
                                <div className="p-3 border-r border-hairline flex items-center">
            ...
            {/* Save indicator */}
            <div className={`absolute bottom-4 right-4 bg-surface-raised border border-hairline-strong text-white text-xs px-3 py-1.5 rounded-md shadow-lg flex items-center gap-2 transition-all duration-300 ${step === 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
