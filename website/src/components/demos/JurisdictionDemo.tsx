...
type VendorId = "firebase" | "supabase" | "rebase";
...
interface Vendor {
    ...
    operator: string;
    operatorSeat: string;
    /** Whether that operator is someone other than the reader. */
    thirdParty: boolean;
...
const ROWS = [
    { key: "region", label: "Where the bytes sit", hint: "all three can answer Europe" },
    ...
    return (
        <div className="frame overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_0.85fr]">
                                    ...
                                    {row.key === "keyholders" ? (
                                                    ...
                                                    <span
                                                        ...
                                                        className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-sm transition-all duration-300 ${
                                                            isYou
                                                                ...
                                                                : "bg-amber-500/[0.07] text-amber-200/90 ring-1 ring-inset ring-amber-500/25"
                                                        }`}>
                                                        {!isYou && (
                                        ...
                                        <p className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
                                            <span className={`text-lg font-semibold transition-colors duration-300 ${
                                                sovereign ? "text-white" : "text-amber-200"
                                            }`}>
                                                {vendor.operator}
                                            </span>
                ...
                {/* ── The read-out ──────────────────────────────────── */}
                <div className={`relative flex flex-col justify-between gap-6 border-t border-surface-800/60 p-6 transition-colors duration-500 lg:border-l lg:border-t-0 sm:p-7 ${
                    sovereign ? "bg-primary/[0.04]" : "bg-amber-500/[0.03]"
                }`}>
                    <div
                        ...
                        style={{
                            background: sovereign
                                ? "radial-gradient(340px 260px at 70% 0%, rgba(0,112,244,0.13), transparent 75%)"
                                : "radial-gradient(340px 260px at 70% 0%, rgba(245,158,11,0.09), transparent 75%)"
                        }}
                    />

                    <div className="relative">
                        <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-medium ring-1 ring-inset transition-colors duration-300 ${
                            sovereign
                            ...
                            {sovereign ? "One party to the data" : "Two parties to the data"}
                        </span>
            ...
            <p className="border-t border-surface-800/60 bg-surface-950/40 px-5 py-3 text-[11px] leading-relaxed text-surface-500 sm:px-7">
                Regions above are the European options each vendor publishes, not defaults — all three will keep data in
